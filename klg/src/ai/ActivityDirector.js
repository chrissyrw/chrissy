export class ActivityDirector {
  ...
  bind(){
    ...
    this.game.events.on('mission:completed',m=>{this.cooldown=8;this.lastActivity=m?.id||this.lastActivity;});
    this.game.events.on('economy:director',d=>{this.economy=d||{};});
    this.game.events.on('business:orders',d=>{this.business=d||{};});
  }
  ...
  score(t){
    const h=this.hour(),w=this.game.state.get().world.weather,ctx=this.playerContext(),d=this.world||{};
    let score=t.base;
    if(h>=22||h<5)score*=t.fit.night||1;
    if(w==='storm')score*=t.fit.storm||1;
    else score*=t.fit.clear||1;
    if((d.congestion||0)>40)score*=t.fit.traffic||1;
    if((d.population||1)>1.15)score*=t.fit.population||1;
    if((this.economy?.rewardScale||1)>1.1)score*=t.fit.economy||1;
    if(ctx.reputation>50)score*=t.fit.reputation||1;
    if(ctx.performance>1)score*=t.fit.performance||1;
    if(ctx.wanted>2&&t.type==='CONTRACT')score*=.55;
    if(this.lastActivity===t.id)score*=.25;
    return score;
  }
  choose(){
    const ranked=this.templates.map(t=>({t,score:this.score(t)})).sort((a,b)=>b.score-a.score);
    if(!ranked.length)return null;
    const pool=ranked.slice(0,3),pick=pool[Math.floor(Math.random()*pool.length)];
    return {...pick.t,score:Math.round(pick.score),issuedAt:this.hour()};
  }
  issue(){
    if(this.cooldown>0)return;
    const mission=this.game.state.get().mission;
    if(mission?.active)return;
    const activity=this.choose();
    if(!activity)return;
    this.current=activity;this.lastActivity=activity.id;this.cooldown=16;
    this.game.events.emit('activity:director',activity);
    const el=document.querySelector('#activity-ai');if(el)el.textContent=`ACTIVITY AI · ${activity.district.toUpperCase()} · ${activity.title}`;
  }
