export class ProgressionSystem {
  ...
  onMissionCompleted(m){
    ...
    const newlyUnlocked=!!(location&&!unlocks.includes(location));
    if(newlyUnlocked)unlocks.push(location);
    ...
    this.game.events.emit('progression:updated',{xp,level,oldLevel,deltaXp:bonus,unlocked:newlyUnlocked?location:null});
    if(level>oldLevel)this.game.events.emit('progression:level-up',{level,unlocked:newlyUnlocked?location:null});
