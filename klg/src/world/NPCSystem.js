...
export class NPCSystem {
  constructor(game){
    ...
    this.maxPopulation=140;this.activePopulation=88;this.roadGraph=null;this.navRevision=0;this.populationTargets={};this.busStops=[];this.buses=[];this.motoStages=[];this.shops=[];this.groups=[];this.directorTimer=0;
    this.bindRoadGraph();this.buildPedestrianInfrastructure();this.buildTransitLife();this.buildShops();this.createPopulation();this.bindEvents();
  }
  ...
  bindRoadGraph(){this.game.events.on('geo:road-graph-ready',graph=>{this.roadGraph=graph;this.navRevision++;});}
  applyNavigationEvent(e={}){const district=String(e.district||'').toUpperCase();for(const n of this.npcs){if(!n.active)continue;if(district&&String(n.district||'').toUpperCase()!==district)continue;const target=this.roadGraph?.nearest(n.targetX,n.targetZ);if(target){const start=this.roadGraph.nearest(n.mesh.position.x,n.mesh.position.z);const path=this.roadGraph.findPath(start,target);if(path.length>1){n.navPath=path.slice(1);n.navIndex=0;n.navRevision=this.navRevision;}}}}
  bindEvents(){this.game.events.on('city:event',e=>this.onCityEvent(e));this.game.events.on('vehicle:damage',()=>this.broadcastReaction('accident'));this.game.events.on('wanted:changed',d=>{if((d?.level||0)>0)this.broadcastReaction('police');});this.game.events.on('population:director',d=>this.applyPopulationTargets(d));this.game.events.on('emergency:npc-impact',e=>this.onEmergencyImpact(e));this.game.events.on('npc:navigation-reroute',e=>this.applyNavigationEvent(e));this.game.events.on('npc:navigation-opportunity',e=>this.applyNavigationEvent(e));this.game.events.on('npc:mission-impact',e=>this.applyMissionImpact(e));}
  applyMissionImpact(e={}){const district=String(e.district||'').toUpperCase();const impact=Number(e.impact||0);for(const n of this.npcs){if(!n.active)continue;if(district&&String(n.district||'').toUpperCase()!==district)continue;n.mood=impact>=0?'positive':'uneasy';n.lastEvent=6;if(impact<0){n.state='flee';}else if(Math.random()<.35){n.state='shop';}}}
  ...
  update(dt){const time=this.game.state.get().world.time,player=this.game.vehicles.active?.mesh||this.game.player;this.directorTimer+=dt;if(this.directorTimer>3){this.directorTimer=0;this.refreshGroups();}
    ...
    for(const n of this.npcs){if(!n.active)continue;n.decision-=dt;n.lastEvent=Math.max(0,n.lastEvent-dt);n.wait=Math.max(0,n.wait-dt);if(n.decision<=0){n.decision=2.5+Math.random()*4;this.chooseTarget(n,time);}
      ...
      let target;if(n.navPath?.length){const wp=n.navPath[n.navIndex||0];target=new THREE.Vector3(wp.x,0,wp.z);if(n.mesh.position.distanceTo(target)<1.4){n.navIndex=(n.navIndex||0)+1;if(n.navIndex>=n.navPath.length)n.navPath=null;}}if(!target)target=new THREE.Vector3(n.targetX,0,n.targetZ);let dir=target.clone().sub(n.mesh.position).setY(0),distance=dir.length();if(n.state==='cross'){if(!this.trafficSafe(n)){n.wait=.6;dir.set(0,0,0);}else if(distance<1.1){n.state='wander';n.crossing=null;n.decision=0;}}
