...
import { StreetLevelPresentationSystem } from './StreetLevelPresentationSystem.js';
...
export class RwandaGeoSystem{
  constructor(game){this.game=game;this.db=new RwandaGeoDatabase();this.graph=new RoadGraph();this.group=new THREE.Group();this.game.scene.add(this.group);this.streetPresentation=new StreetLevelPresentationSystem(game);this.loaded=false;this.loading=false;this.origin={lon:30.0619,lat:-1.9441};this.scale=0.11;this.loadKigaliRoads();}
  ...
  async loadKigaliRoads(){
    ...
    try{
      ...
      const count=this.db.ingestGeoJSON(data);this.graph.build(this.db.roads,(x,y)=>this.project(x,y));this.renderRoads();this.streetPresentation.build(this.db.roads,(x,y)=>this.project(x,y));this.loaded=true;this.game.events.emit('geo:roads-loaded',{count,nodes:this.graph.nodes.size,edges:this.graph.edges.size,source:this.db.source});this.game.events.emit('geo:road-graph-ready',this.graph);
  ...
  update(dt){this.streetPresentation?.update(dt);}
  dispose(){this.group.clear();this.streetPresentation?.dispose();}
}
