export class CompletionAuditSystem{
 ...
 bind(){
  ...
  const valid=(name,test,msg)=>this.game.events.on(name,e=>test(e||{})?ok(name):this.fail(msg));
  valid('crewFormed',e=>e.id||e.crewId,'crew:formed missing id');
  valid('crewProgress',e=>e.id||e.crewId,'crew:progress missing id');
  valid('safehouseUpgrade',e=>e.crewId||e.id,'safehouse upgrade missing crew id');
  valid('rivalry',e=>e.district||e.crewA||e.crewB,'rivalry missing district/crews');
  valid('crewMissionResolved',e=>e.crewId||e.crewMissionId,'crew mission resolution missing id');
  valid('opportunitySeen',e=>e.id&&e.district,'opportunity missing id/district');
  valid('opportunityAccepted',e=>e.id||e.opportunityId||e.missionId,'opportunity accept missing id');
  valid('missionStarted',e=>e.id,'mission start missing id');
  valid('missionCompleted',e=>e.id,'mission completion missing id');
  valid('missionFailed',e=>e.id,'mission failure missing id');
  valid('objectiveProgress',e=>e.missionId&&e.type,'objective progress missing mission/type');
  valid('objectiveComplete',e=>e.missionId,'objective completion missing missionId');
 }
