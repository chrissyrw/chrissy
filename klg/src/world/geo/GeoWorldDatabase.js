...
export class GeoWorldDatabase{
 constructor(){this.buildings=[];this.pois=[];this.districts=[];this.terrain=[];this.source='KLG Geo World v3';}
 ingestOSM(json){const elements=json?.elements||[];for(const e of elements){const tags=e.tags||{};const kind=KIND_MAP[tags.building?'building':tags.shop?'shop':tags.amenity?'amenity':tags.highway?'highway':tags.tourism?'tourism':tags.leisure?'leisure':tags.natural?'natural':''];const lat=e.lat??e.center?.lat,lon=e.lon??e.center?.lon;if(lat==null||lon==null||!kind)continue;if(kind==='BUILDING')this.buildings.push({id:e.id,lat,lon,kind,tags,geometry:e.geometry||null});else if(kind!=='ROAD')this.pois.push({id:e.id,lat,lon,kind,name:tags.name||'',tags});}return{buildings:this.buildings.length,pois:this.pois.length};}
